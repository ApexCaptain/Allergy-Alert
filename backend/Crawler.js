...
class Crawler {
  ...
  async getData(
    ...
    for (const eachRow of dataToReturn) {
      ...
      await KoconutArray.from(eachRow.menu)
        .onEach(
          (eachMenu) =>
            new Promise((resolve) => {
              this.database.get(
  ...
  async changeMealPeriod(mealPeriod) {
    return new Promise(async (resolve) => {
      if (this.currentMealPeriod != mealPeriod) {
        const inputFrame = await (
          await this.page.$("html > frameset > frame:nth-child(1)")
        ).contentFrame();
        await inputFrame.select(
          "body > form > table > tbody > tr:nth-child(3) > td > table > tbody > tr > td:nth-child(4) > select",
          mealPeriod
        );
        await inputFrame.click(
          "body > form > table > tbody > tr:nth-child(3) > td > table > tbody > tr > td:nth-child(8) > input[type=button]"
        );
  ...
  async crawl(weekDay, passedMealPeriod) {
    await this.changeMealPeriod(passedMealPeriod);

    const tableFrame = await (
      await this.page.$("html > frameset > frame:nth-child(2)")
    ).contentFrame();

    let menu = await tableFrame.evaluate((weekDay) => {
      const rst = Array.from(
        document.querySelectorAll(
          `body > table > tbody > tr:nth-child(1) > td > table > tbody > tr:nth-child(${
            weekDay + 2
          }) > td:nth-child(${weekDay == 1 ? 4 : 3}) > table > tbody > tr > td`
        )
      );
      return rst
        .map((rst) => rst.innerText)
        .filter((eachMenu) => !eachMenu.includes("included"));
    }, weekDay);
    let price = 0;
    if (passedMealPeriod == mealPeriod.Launch) {
      price =
        menu.length == 0
          ? 0
          : await tableFrame.$eval(
              `body > table > tbody > tr:nth-child(1) > td > table > tbody > tr:nth-child(${
                weekDay + 2
              }) > td:nth-child(${weekDay == 1 ? 5 : 4})`,
              (el) => {
